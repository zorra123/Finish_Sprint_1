...
private:
    ...
    //template <typename Status >
    vector<Document> FindAllDocuments(const Query& query, DocumentStatus status_in) const {

        return FindAllDocuments(query, [status_in](int document_id, DocumentStatus status, int rating) { return status == status_in; });
